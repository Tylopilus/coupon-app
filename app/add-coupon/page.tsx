...
import { getStores, addCoupon, Coupon } from '../utils/storage'
...
export default function AddCoupon() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (type === 'checkbox') {
      setFormData(prev => {
        console.log('Updating formData (checkbox)', name, e.target.checked);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!newCoupon.store || !newCoupon.code || !newCoupon.discount || !newCoupon.expiryDate) {
      ...
      setError(error instanceof Error ? error.message : 'An unknown error occurred while adding the coupon. Please try again.')
    } finally {
      setIsProcessing(false)
    }
  }

  console.log('AddCoupon component rendered', { stores, formData, isLoading, error });

  if (isLoading) {
    return <div className="container mx-auto p-4 max-w-[390px]">Loading...</div>
  }
  ...
  return (
    <div className="container mx-auto p-4 max-w-[390px]">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Add New Coupon</h1>
      </div>
      <form onSubmit={handleSubmit} className="space-y-4 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
        <div>
          <label htmlFor="store" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Store</label>
        ...
        <div>
          <label htmlFor="newStore" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">New Store</label>
          <div className="flex items-center space-x-2">
            <input
              type="text"
              id="newStore"
        ...
        <div>
          <label htmlFor="discount" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Discount</label>
          <input
            type="text"
            id="discount"
            name="discount"
        ...
        <div className="flex items-center mb-4">
          <input
            ...
            onChange={(e) => setFormData(prev => ({ ...prev, expiryDate: e.target.checked ? 'No Expiry' : '' }))}
