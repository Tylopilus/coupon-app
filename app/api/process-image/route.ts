...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const base64Data = imageData.startsWith('data:image') ? imageData.split(',')[1] : imageData;
    ...
    if (!response || !response.content || response.content.length === 0) {
      ...
      return NextResponse.json({ error: 'Empty response from Anthropic API' }, { status: 500 });
    }

    const content = response.content[0];
    if (!content || typeof content.text !== 'string') {
      console.error('Invalid content in Anthropic API response');
      return NextResponse.json({ error: 'Invalid content in Anthropic API response' }, { status: 500 });
    }

    const result = JSON.parse(content.text);
    if (!result || typeof result !== 'object' || (!result.code && !result.discount && !result.store && !result.expiryDate)) {
      console.error('Invalid JSON format in API response');
      return NextResponse.json({ error: 'Invalid JSON format in API response' }, { status: 500 });
    }
